...
| `SiteBody.tsx` | Server | Composes `LoadingScreen` + `Header` + `<main>` + `Footer` + `ChatBubble` + the site-wide LocalBusiness JSON-LD, given a `locale`. Both root layouts (`(en)/layout.tsx`, `(ne)/ne/layout.tsx`) render this inside their `<body>` — see [architecture.md](./architecture.md). |
| `LoadingScreen.tsx` | Client | Full-screen white overlay with a live load percentage, shown on every page until `window.load` fires (tracked via `document.images` load/error counts, with a `document.readyState === "complete"` fallback for when `load` already fired before mount). Hides itself (`return null`) once done — no flag/prop, purely client-side state. |
...
| `ProductItemList.tsx` | Server | Renders `productGroups[slug]` (from `src/data/products.ts`) as a native `<details>`/`<summary>` accordion, one per group, each item showing its name, size/variant, and an optional thumbnail (`item.image`, via `next/image`, decorative so `alt=""`). No client JS. Used on `CategoryView.tsx`; see [content-editing.md](./content-editing.md#itemized-stock-lists-per-category). |
