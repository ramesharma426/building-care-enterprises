...
The owner has supplied real item-level stock lists — hardware/furniture fittings, wire mesh, plain sheets, machinery, and steel section items (2026-08-26), a full paint & coatings list grouped by brand (2026-08-30), and an Om Shivam (Laxmi Technoplast) CPVC/uPVC pipe & fittings list (2026-09-08) — transcribed as-is in [`docs/product-inventory.md`](./product-inventory.md) and structured into `src/data/products.ts` as `productGroups: Partial<Record<CategorySlug, ProductGroup[]>>` (each group has a `heading` and `items: { name, variant?, image? }[]`).

`image?` is an optional path under `public/`, one per item (fitting/pipe *type* — reused across all of that type's size variants, not one image per size). Only `sanitary-plumbing` has these so far: the 51 thumbnails under `public/products/sanitary-plumbing/*.jpg` were cropped from the manufacturer's own product photos on the Om Shivam price-list scans the owner sent (first-party photos of the exact branded product — safer to reuse than a random web image, and more accurate). If a future list doesn't include its own product photos, ask before sourcing images from elsewhere online (licensing for commercial use needs checking case by case).
...
`building-materials` (4 groups: Hardware & Fittings, Wire Mesh, Plain Sheets, Steel Sections), `machinery-tools` (1 group: Machinery), `paints` (13 groups, one per brand — Luxol/Lukol, Brolac, Jensolin, BP, Weathercoat, Silk, Easy Clean, Rangoli, Walmasta, Bison, REWA, Tata, Aagaman), and `sanitary-plumbing` (4 groups: CPVC Pipes, CPVC Fittings, uPVC Pipes, uPVC Fittings) have real itemized data — nothing in the owner's lists maps to `appliances` yet. Item names are kept as-is (untranslated) in both locales since they're trade names/romanized terms, not marketing copy — `ProductItemList` reads directly from `products.ts`, not the dictionaries. Note the source sheets themselves are inconsistent between "Luxol" and "Lukol" for the same product line (likely a labeling quirk on the owner's side, not a transcription error) — kept as printed rather than "corrected." Only item names and available sizes were carried over from the Om Shivam sheet — the site never shows pricing anywhere, in copy or docs, so no other detail from that sheet is repeated.
